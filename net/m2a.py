...
#     pad = torch.zeros_like(x).to(x.device)[:, :,:amount] 
#     if type == 'left':
#         xt2 = torch.cat([x.clone()[:,:, amount:], pad], 2)  
#     elif type == 'right':
#         xt2 = torch.cat([pad, x.clone()[:,:, :-amount]], 2)
#     xt2.to(x.device)  
#     return xt2
...
#     elif type == 'right':
#         xt2 = torch.cat([pad, x.clone()[:,:,:, :-amount]], 3)
#     xt2.to(x.device) 
#     return xt2
...
def compute_shift(x, type, amount=1):  
    ...
    if type == 'left':
        xt2 = torch.cat([x.clone()[:,:, amount:], pad1], 2)  
        xv3 = torch.cat([x.clone()[:,:,:,amount:], pad2], 3)
    elif type == 'right':
        xt2 = torch.cat([pad1, x.clone()[:,:, :-amount]], 2)
        xv3 = torch.cat([pad2, x.clone()[:, :,:,:-amount]], 3)
    xt2.to(x.device)  
    xv3.to(x.device)
    return xt2, xv3
...
class CustomMotionAttention(nn.Module):
    def __init__(self, n_segment, einops_to, motion_mod, in_channels):
        ...
        self.name = 'attn'
        self.n_segment = n_segment
        # self.einops_from = 'bt c h w' 
        # self.einops_to = 'bt h (c w)' 
    ...
    def forward(self, x):
        ...
        # attention -- don't use Attention class bc of layernorm  self.einops_from = '(b t) c h w'  einops_to=b t (c h w)
        xn = rearrange(xn, f'{self.einops_from} -> {self.einops_to}', t=self.n_segment, c=c, h=h, w=w)
        # xn = rearrange(xn, f'{self.einops_from} -> {self.einops_to}', bt=bt, c=c, h=h, w=w) 
        x_attn, _ = self_attention(xn, xn, xn) 
        # x_attn = rearrange(x_attn, f'{self.einops_to} -> {self.einops_from}',  bt=bt, c=c, h=h, w=w)  
...
class Motion(nn.Module):
    ...
    def forward(self, x):  
        ...
        # xt2= compute_shift(x, type='left') - x  
