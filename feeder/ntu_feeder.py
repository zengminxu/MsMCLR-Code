...
class Feeder_triple(torch.utils.data.Dataset):
    ...
    def _strong_aug(self, data_numpy):
        ...
        if '2' in self.aug_method:
            data_numpy = tools.random_rotate(data_numpy)  # rotation
        ...
        #     data_numpy = tools.random_time_flip(data_numpy)  # reverse
        if '6' in self.aug_method:
            data_numpy = tools.noise_joint_fix(data_numpy)  # reverse
