    ...
    def train(self, epoch):
        ...
        for [data1, data2, data3], label in loader:
            ...
            if epoch <= self.arg.mining_epoch: 
                output1, target1, output2, output3,output4, target2 = self.model(data1, data2, data3)  
                ...
                loss4 = -torch.mean(torch.sum(torch.log(output4) * target2, dim=1))  # DDM loss  Ld2
                loss = loss1 + (loss2 + loss3+loss4) / 3.
            else: 
                output1, mask, output2, output3,output4, target2 = self.model(data1, data2, data3, nnm=True, topk=self.arg.topk)  
                if hasattr(self.model, 'module'):
                    self.model.module.update_ptr(output1.size(0))
                else:
                    self.model.update_ptr(output1.size(0))
                ...
                loss4 = -torch.mean(torch.sum(torch.log(output4) * target2, dim=1))
                loss = loss1 + (loss2 + loss3+loss4) / 3. 
            ...
            # self.iter_info['group_contrastive_loss'] = group_contrastive_loss.data.item()
            # group_contrastive_loss_value.append(self.iter_info['group_contrastive_loss'])
            self.show_iter_info()
